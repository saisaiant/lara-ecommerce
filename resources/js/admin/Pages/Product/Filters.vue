...
<template>
    <Card class="mb-4">
        ...
        <form class="grid grid-cols-4 gap-8">
            ...
            <SelectGroup v-if="subCategories.length > 0" label="Sub Category" v-model="filters.subCategoryId"
                :items="subCategories" />
            <SelectGroup label="Featured" v-model="filters.featured"
                :items="[{ id: 1, name: 'Yes' }, { id: 0, name: 'No' }]" />
            <SelectGroup label="Show on slider" v-model="filters.showOnSlider"
                :items="[{ id: 1, name: 'Yes' }, { id: 0, name: 'No' }]" />
